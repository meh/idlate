...
defnumeric Error.NoSuckNick, 401, [:nick] do
...
defnumeric Error.NoSuckServer, 402, [:server] do
...
defnumeric Error.NoSuchChannel, 403, [:channel] do
...
defnumeric Error.CannotSendToChan, 404, [:channel] do
  def to_string(__MODULE__[channel: channel]) do
    "#{channel} :Cannot send to channel"
  end
end

defnumeric Error.YouNeedVoice, 404, [:channel] do
  def to_string(__MODULE__[channel: channel]) do
    "#{channel} :You need voice (+v) (#{channel})"
  end
end

defnumeric Error.NoExternalMessages, 404, [:channel] do
  def to_string(__MODULE__[channel: channel]) do
    "#{channel} :No external channel messages (#{channel})"
  end
end

defnumeric Error.YouAreBanned, 404, [:channel] do
  def to_string(__MODULE__[channel: channel]) do
    "#{channel} :You are banned (#{channel})"
  end
end

defnumeric Error.NoCTCPs, 404, [:channel] do
  def to_string(__MODULE__[channel: channel]) do
    "#{channel} :CTCPs are not permitted in this channel (#{channel})"
  end
end

defnumeric Error.NoColors, 404, [:channel] do
  def to_string(__MODULE__[channel: channel]) do
...
defnumeric Error.TooManyChannels, 405, [:name] do
...
# Sent to a user when they have joined the maximum number of allowed channels and they try to join another channel.
defnumeric Error.WasNoSuchNick, 406, [:nick] do
  def to_string(__MODULE__[nick: nick]) do
...
defnumeric Error.TooManyTargets, 407, [:recp] do
...
defnumeric Error.NoOrigin, 409, [] do
  def to_string(__MODULE__[]) do
    ":No origin specified"
  end
end

defnumeric Error.NoRecipient, 411, [:recp] do
  def to_string(__MODULE__[recp: recp]) do
    ":No recipient given (#{recp})"
  end
end

defnumeric Error.NoTextToSend, 412, [] do
  def to_string(_) do
    ":No text to send"
  end
end

defnumeric Error.NoTopLevel, 413, [:mask] do
...
defnumeric Error.WildTopLevel, 414, [:mask] do
  def to_string(__MODULE__[mask: mask]) do
...
defnumeric Error.UnknownCommand, 421, [:command] do
...
defnumeric Error.NoMOTD, 422, [] do
  def to_string(__MODULE__[]) do
...
defnumeric Error.NoAdminInfo, 423, [:server] do
  def to_string(__MODULE__[server: server]) do
...
defnumeric Error.FileError, 424, [:file_op, :file] do
...
defnumeric Error.NickCollision, 436, [:nick] do
  def to_string(__MODULE__[nick: nick]) do
...
defnumeric Error.UserNotInChannel, 441, [:nick, :channel] do
...
defnumeric Error.NotOnChannel, 442, [:channel] do
  def to_string(__MODULE__[channel: channel]) do
...
defnumeric Error.UserOnChannel, 443, [:nick, :channel] do
  def to_string(__MODULE__[nick: nick, channel: channel]) do
...
defnumeric Error.NoLogin, 444, [:user] do
...
defnumeric Error.SummonDisabled, 445, [] do
  def to_string(_) do
...
defnumeric Error.UsersDisabled, 446, [] do
  def to_string(_) do
...
defnumeric Error.NotRegistered, 451, [] do
  def to_string(_) do
...
defnumeric Error.NeedMoreParams, 461, [:command] do
  def to_string(__MODULE__[command: command]) do
...
defnumeric Error.AlreadyRegistred, 462, [] do
  def to_string(_) do
...
defnumeric Error.NoPermForHost, 463, [] do
  def to_string(_) do
...
defnumeric Error.PasswdMismatch, 464, [] do
  def to_string(_) do
    ":Password incorrect"
  end
end
...
defnumeric Error.YoureBannedCreep, 465, [] do
  def to_string(_) do
    ":You are banned from this server"
  end
end

defnumeric Error.KeySet, 467, [:channel] do
  def to_string(__MODULE__[channel: channel]) do
    "#{channel} :Channel key already set"
  end
end

defnumeric Error.ChannelIsFull, 471, [:channel] do
  def to_string(__MODULE__[channel: channel]) do
    "#{channel} :Cannot join channel (+l)"
  end
end

defnumeric Error.UnknownMode, 472, [:mode] do
  def to_string(__MODULE__[mode: mode]) do
    "#{mode} :is unknown mode char to me"
  end
end

defnumeric Error.InviteOnlyChan, 473, [:channel] do
  def to_string(__MODULE__[channel: channel]) do
    "#{channel} :Cannot join channel (+i)"
  end
end

defnumeric Error.BannedFromChan, 474, [:channel] do
  def to_string(__MODULE__[channel: channel]) do
    "#{channel} :Cannot join channel (+b)"
  end
end

defnumeric Error.BadChannelKey, 475, [:channel] do
  def to_string(__MODULE__[channel: channel]) do
    "#{channel} :Cannot join channel (+k)"
  end
end

defnumeric Error.NoKnock, 480, [:channel, :reason] do
...
defnumeric Error.NoPrivileges, 481, [] do
  def to_string(_) do
...
defnumeric Error.ChanOPrivsNeeded, 482, [:user] do
  def to_string(__MODULE__[user: user]) do
...
defnumeric Error.CantKillServer, 483, [] do
  def to_string(_) do
...
defnumeric Error.NoOperHost, 491, [] do
  def to_string(_) do
...
defnumeric Error.UmodeUnknownFlag, 501, [] do
  def to_string(_) do
    ":Unknown MODE flag"
  end
end
...
defnumeric Error.UsersDontMatch, 502, [] do
  def to_string(_) do
...
# custom
defnumeric Error.BadChanmask, 476, [:channel] do
  def to_string(__MODULE__[channel: channel]) do
    "#{channel} :Bad channel name"
  end
end

defnumeric Error.AllMustUseSSL, 974, [] do
  def to_string(_) do
    "z :all members must be connected via SSL"
  end
end

defnumeric Error.SSLRequired, 489, [:channel] do
  def to_string(__MODULE__[channel: channel]) do
    "#{channel} :Cannot join channel (SSL is required)"
  end
end

defnumeric Error.NoNickChange, 447, [:channel] do
  def to_string(__MODULE__[channel: channel]) do
    "Can not change nickname while on #{channel} (+N)"
  end
end

defnumeric Error.NoKicks, 972, [] do
  def to_string(_) do
    "KICK :channel is +Q"
  end
end

defnumeric Error.NoInvite, 518, [:channel] do
  def to_string(__MODULE__[channel: channel]) do
